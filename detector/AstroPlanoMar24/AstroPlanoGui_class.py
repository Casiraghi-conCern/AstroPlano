...
import AstroSensori
#import astro_detector_rp as a_d_p
...
# label file configurazione
   ...
   # entry per il nome file di config.
    ...
    #guadagno basso/alto
        ...
        #modalità normale/angolare
        self.s_mode = tk.IntVar()
        self.s_mode.set(0)
        self.r_mode_norm= tk.Radiobutton(self.lf_record, text="Normal", value =0, padx=10, pady=2,
           width=self.wb, height=self.hb, bg=bkgnd, fg=forgnd, 
           indicatoron=False, activebackground='red', variable=self.s_mode,
           activeforeground=bkgnd, selectcolor='green', anchor='w')
        self.r_mode_norm.grid(row=10, column=0,sticky=tk.W,columnspan=6)
        
        self.r_mode_ang= tk.Radiobutton(self.lf_record, text="Angular", value =1, padx=10, pady=2,
           width=self.wb, height=self.hb, bg=bkgnd, fg=forgnd, 
           indicatoron=False, activebackground='red', variable=self.s_mode,
           activeforeground=bkgnd, selectcolor='green', anchor='w')
        self.r_mode_ang.grid(row=10, column=1, sticky=tk.W, columnspan=6)
...
# come tensioni prendo quelle corrette per la temperatura FFX             
        ...
        A_S.astrodata.mode =  int(self.s_mode.get()) # modalità norm/ang
        ...
        with open(A_S.astrodata.configfilepath+ A_S.astrodata.configfile) as fp:
            ...
            for line in fp:
                ...
                if parole[0]== 'leading':
                    ...
                    A_S.astrodata.mode = int(parole[1])
                    self.s_mode.set(A_S.astrodata.mode)    
...
#        global file_configurazione
        ...
        A_S.astrodata.mode =  int(self.s_mode.get())  
...
# e delle temperature se abilitato
    if A_S.real_sensors:
        ...
        #aggiornaMessaggio ("\nVbat: %6.1f , Vrp: %6.1f , Vp1: %6.1f , Vp2: %6.3f \n" % (AstroSensori.APLsens.V_values(0), AstroSensori.APLsens.V_values(1), AstroSensori.APLsens.V_values(2), AstroSensori.APLsens.V_values(3)) )  
        if AstroSensori.APLsens.V_values(0) < V_crit: 
                aggiornaMessaggio ("\n***   Attenzione!!!  Batteria quasi Esaurita !!! Inizio spegnimento controllato ***\n") 
                GPIO.cleanup()
                detect.astro_detector.PowerOff()
