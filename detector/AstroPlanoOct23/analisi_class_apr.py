...
# ritorna -1 se l'evento e' completo e non e' stato possibile aggiungere un altro trigger        
    ...
    # controllo che in effetti il pezzo di trigger sia +/- contemporaneo al candidato
        ...
        if ch >11:                  # FFX dovrebbe essere inutile
            debug=1
...
#            errore qui devi inserire il trigger che hai appena trovato
            #azzero  l'evento appena scritto e inserisco il trigger appena letto
            self.scintil_active=[0]* A_S.astrodata.N_SiPM
            self.trigs.clear()
            self.trigs= list() #pronti per il prossimo
            # metto il primo che ho trovato
            self.trigs.append(copy.deepcopy(lastTrigger))
            chan = lastTrigger.channel
...
# si completa un blocco temporale     
    leftover=0 
    ntot = 0
    while (ntot <  N_eventi) and not A_S.astrodata.stop_analisi :
       if ntot%20000 == 0: 
